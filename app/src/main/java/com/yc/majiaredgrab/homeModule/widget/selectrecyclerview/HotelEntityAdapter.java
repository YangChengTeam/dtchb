package com.yc.majiaredgrab.homeModule.widget.selectrecyclerview;
...
public class HotelEntityAdapter extends SectionedRecyclerViewAdapter<HeaderHolder, DescHolder, RecyclerView.ViewHolder> {
    ...
    protected int getItemCountForSection(int section) {
        ...
        return CommonUtils.isEmpty(allTagList.get(section).tagInfoList) ? 0 : count;
    }
...
//        holder.openView.setText(mBooleanMap.get(section) ? "关闭" : "展开");//true显示关闭字段，false显示展开字段
    ...
    protected void onBindItemViewHolder(DescHolder holder, final int section, final int position) {
       // holder.descView.setText(allTagList.get(section).tagInfoList.get(position).tagName);
        if (!TextUtils.isEmpty(allTagList.get(section).tagInfoList.get(position).tagName)){
            holder.descView.setText(allTagList.get(section).tagInfoList.get(position).tagName);
        }
        if (!TextUtils.isEmpty(allTagList.get(section).tagInfoList.get(position).getTitle())){
            holder.title.setText(allTagList.get(section).tagInfoList.get(position).getTitle());
        }else {
