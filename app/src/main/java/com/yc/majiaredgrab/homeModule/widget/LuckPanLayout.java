...
public class LuckPanLayout extends RelativeLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= CommonUtils.dip2pxss(context,10)*2;
    ...
    private void drawSmallCircle(boolean FirstYellow){
        int pointDistance = radius - CommonUtils.dip2pxss(context,10);
        for(int i=0;i<=360;i+=20){
            int x = (int) (pointDistance * Math.sin(CommonUtils.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(CommonUtils.change(i))) + CircleY;

            if(FirstYellow)
                canvas.drawCircle(x,y,CommonUtils.dip2pxss(context,4),yellowPaint);
            else
                canvas.drawCircle(x,y,CommonUtils.dip2pxss(context,4),whitePaint);
