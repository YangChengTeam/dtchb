...
public interface PersonApi {
    ...
    Flowable<HttpResult<WeixinCashBeans>> cashMoney(@Field("group_id")String groupId, @Field("paystype") String wx, @Field("amount")String cashMoney,@Field("imei")String imei,@Field("is_new_ver")String is_new_ver);
    ...
    @POST("v1.user/bonusesupd")
    @FormUrlEncoded
    Flowable<HttpResult<TaskUnLockResBeans>> getbaijingUnlockTask(@Field("imei")String imei, @Field("group_id") String group_id, @Field("other_id")String baijinunLockTaskId);

    @POST("v1.user/bonusescash")
    @FormUrlEncoded
    Flowable<HttpResult<EmptyBeans>> bonusescash(@Field("imei")String imei, @Field("group_id")String group_id,@Field("is_new_ver")String is_new_ver);

    @POST("v1.user/getsignedcash")
    @FormUrlEncoded
    Flowable<HttpResult<SignWiths>> getSignMoenys(@Field("imei")String imei, @Field("group_id")String group_id);
}
