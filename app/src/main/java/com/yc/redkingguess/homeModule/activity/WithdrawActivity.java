...
import com.yc.redkingguess.homeModule.adapter.DisposeMoneyAdapter;
import com.yc.redkingguess.homeModule.adapter.DisposeNoticeAdapter;
import com.yc.redkingguess.homeModule.contact.WithdrawConstact;
import com.yc.redkingguess.homeModule.fragment.DisposeTintFragment;
import com.yc.redkingguess.homeModule.fragment.ExitTintFragment;
...
import com.yc.redkingguess.homeModule.module.bean.WithDraw5Beans;
import com.yc.redkingguess.homeModule.present.WithdrawPresenter;
import com.yc.redkingguess.homeModule.widget.TextViewSwitcher;
...
public class WithdrawActivity extends BaseActivity<WithdrawPresenter> implements WithdrawConstact.View {
                        ...
                        if (userCash >= money) {//可提现
                             if (i==1){
                                 ...
                                 if (isSign==0){//没有签到7天
                                     ...
                                     if (invite_meet_num>=5){
                                         mPresenter.getSignMoeny(CacheDataUtils.getInstance().getUserInfo().getImei(),CacheDataUtils.getInstance().getUserInfo().getGroup_id());
                                     }else {
                                         showSigin7();
                                     }
                                 }
    ...
    public void getWithDrawDataSuccess(TithDrawBeans data) {
        ...
        invite_meet_num = user_other.getInvite_meet_num();
    ...
    @Override
    public void getSignMoenysSuccess(SignWiths benas) {
        if (benas!=null&&!TextUtils.isEmpty(benas.getMoney())){
            showDialogRedHong();
        }else {
            setDialogs(8,"");
        }
...
//            return;
    ...
    private void showSigin7() {//invite_meet_num
        CacheDataUtils.getInstance().setLevel("1");
        snatchDialogs = new SnatchDialog(this);
        View builder = snatchDialogs.builder(R.layout.dialog_sign7_item);
        TextView tv_sure = builder.findViewById(R.id.tv_sure);
        TextView tv_des=builder.findViewById(R.id.tv_des);
        int invites=5-invite_meet_num;
        tv_des.setText("仅需邀请"+invites+"个用户即可获得打款资格");
    ...
    private SnatchDialog snatchHongb;
    private ImageView iv_cancle;
    private FrameLayout fl_ad_containerss;
    private void showDialogRedHong() {
        CacheDataUtils.getInstance().setLevel("1");
        snatchHongb = new SnatchDialog(this);
        View builder = snatchHongb.builder(R.layout.dialog_redhong_item);
        iv_cancle = builder.findViewById(R.id.iv_cancle);
        if (snatchHongb!=null){
            if (iv_cancle!=null){
                iv_cancle.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        snatchHongb.setDismiss();
                    }
                });
            }
            if (!CommonUtils.isDestory(WithdrawActivity.this)) {
    ...
    private void initDialogRedHong(){
        CacheDataUtils.getInstance().setLevel("1");
        snatchHongb = new SnatchDialog(this);
        View builder = snatchHongb.builder(R.layout.dialog_redhong_item);
        iv_cancle = builder.findViewById(R.id.iv_cancle);
    ...
    private void loadVideoss(){
        final AdPlatformSDK adPlatformSDK = AdPlatformSDK.getInstance(this);
        adPlatformSDK.loadExpressAd(this,"ad_withdraws", 300,200,new AdCallback() {
            @Override
            public void onDismissed() {
