...
import com.yc.redkingguess.homeModule.contact.GrabRedEvenlopesContact;
...
import com.yc.redkingguess.homeModule.present.GrabRedEvenlopesPresenter;
import com.yc.redkingguess.homeModule.widget.SignView;
import com.yc.redkingguess.homeModule.widget.SimpleComponentTwo;
...
import com.yc.redkingguess.utils.ClickListenName;
import com.yc.redkingguess.utils.ClickListenNameTwo;
import com.yc.redkingguess.utils.CommonUtils;
import com.yc.redkingguess.utils.PermissionHelper;
...
//            return;
    ...
    private SnatchDialog snatchSignDialogs;
    private void showSigin7() {
        CacheDataUtils.getInstance().setLevel("1");
        snatchSignDialogs = new SnatchDialog(this);
        View builder = snatchSignDialogs.builder(R.layout.dialog_sign7_item);
        TextView tv_sure = builder.findViewById(R.id.tv_sure);
        TextView tv_cancle=builder.findViewById(R.id.tv_cancle);
        TextView tv_zhanghu=builder.findViewById(R.id.tv_zhanghu);
        tv_zhanghu.setText(CacheDataUtils.getInstance().getUserInfo().getId()+"");
        tv_cancle.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                snatchSignDialogs.setDismiss();
            }
        });
        tv_sure.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                snatchSignDialogs.setDismiss();
                InvationActivity.InvationJupm(GrabRedEvenlopesActivity.this);
            }
        });
        if (!CommonUtils.isDestory(GrabRedEvenlopesActivity.this)) {
            snatchSignDialogs.setShow();
        }
    }
}