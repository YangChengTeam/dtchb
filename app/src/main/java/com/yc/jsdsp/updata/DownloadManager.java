...
import com.yc.jsdsp.beans.module.beans.UpgradeInfozq;
...
public class DownloadManager extends Service {
    ...
    public static void updateApp(UpgradeInfozq upgradeInfozq) {
        ...
        Log.d("ccc", "--------updateApp: "+upgradeInfozq.getDownUrl());
        DownloadTask task = new DownloadTask.Builder(upgradeInfozq.getDownUrl(), new File(parentDir))
        ...
        task.enqueue(new DownloadListener4WithSpeed() {
            ...
            public void infoReady(@NonNull DownloadTask task, @NonNull BreakpointInfo info, boolean fromBreakpoint, @NonNull Listener4SpeedAssistExtend.Listener4SpeedModel model) {
                UpgradeInfozq upgradeInfozq = (UpgradeInfozq) task.getTag();
                upgradeInfozq.setTotalSize(info.getTotalLength());
                upgradeInfozq.setOffsetSize(info.getTotalOffset());
                upgradeInfozq.setDownloadStatus(DownloadStatus.DOWNLOADING);
                EventBus.getDefault().post(upgradeInfozq);
            }
            ...
            public void progress(@NonNull DownloadTask task, long currentOffset, @NonNull SpeedCalculator taskSpeed) {
                UpgradeInfozq upgradeInfozq = (UpgradeInfozq) task.getTag();
                upgradeInfozq.setSpeed(taskSpeed.speed());
                upgradeInfozq.setOffsetSize(currentOffset);
                upgradeInfozq.setDownloadStatus(DownloadStatus.DOWNLOADING);
                EventBus.getDefault().post(upgradeInfozq);
            }
            ...
            public void taskEnd(@NonNull DownloadTask task, @NonNull EndCause cause, @Nullable Exception realCause, @NonNull SpeedCalculator taskSpeed) {
                if (cause == EndCause.COMPLETED) {
                    installSelf();
                    UpgradeInfozq upgradeInfozq = (UpgradeInfozq) task.getTag();
                    upgradeInfozq.setDownloadStatus(DownloadStatus.DOWNLOADED);
                    EventBus.getDefault().post(upgradeInfozq);
                }
            }
        });
    ...
    private  void removeGameApk() {
        if (!TextUtils.isEmpty(parentDir)){
            File fileName = new File(parentDir,"gameload.apk");
            if (fileName != null && fileName.exists() && fileName.isFile()) {
                fileName.delete();
            }
        }
