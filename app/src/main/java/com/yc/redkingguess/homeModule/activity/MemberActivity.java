...
import com.yc.redkingguess.application.MyApplication;
import com.yc.redkingguess.base.BaseActivity;
import com.yc.redkingguess.constants.Constant;
import com.yc.redkingguess.dialog.BottomListDialog;
import com.yc.redkingguess.dialog.LevelDialog;
import com.yc.redkingguess.dialog.NesLoginDialog;
import com.yc.redkingguess.dialog.RedDialogTwo;
import com.yc.redkingguess.dialog.SnatchDialog;
import com.yc.redkingguess.homeModule.adapter.TaskUnlockAdapter;
import com.yc.redkingguess.homeModule.adapter.VipTaskAdapter;
import com.yc.redkingguess.homeModule.contact.MemberConstact;
import com.yc.redkingguess.homeModule.module.bean.BonusesInfoBeans;
...
import com.yc.redkingguess.homeModule.module.bean.UserAccountInfo;
...
import com.yc.redkingguess.homeModule.present.MemberPresenter;
import com.yc.redkingguess.homeModule.widget.SimpleComponentThree;
...
public class MemberActivity extends BaseActivity<MemberPresenter> implements MemberConstact.View {
    ...
    @BindView(R.id.tv_baijinTask)
    TextView tvBaijinTask;
...
//                                receivePacket(redMoney, 2, taskId);
                                    ...
                                    case 8://邀请好友任务
                                        if (status == 0) {
                                            InvationActivity.InvationJupm(MemberActivity.this);
                                        } else if (status == 1) {
                                            String tips = "";
                                            receivePacket(redMoney, 8, taskId, tips);
                                        }
                                        break;
                                }
                            }
        ...
        instance.loadRewardVideoVerticalAd(MemberActivity.this, "ad_member", new AdCallback() {
            ...
            public void onDismissed() {
                if (videoType == 1) {
                    ...
                    if (isVideoClick) {//点击视频
                        ...
                        mPresenter.getbaijinUnlockTask(CacheDataUtils.getInstance().getUserInfo().getImei(), CacheDataUtils.getInstance().getUserInfo().getGroup_id(), baijinunLockTaskId);
                    } else {
                        showjiesuoTaskError();
                    }
                } else {
            ...
            public void onPresent() {
                ...
                if (videoType != 1) {
                    if (videoType == 4) {
                        if (!CommonUtils.isDestory(MemberActivity.this)) {
                            ToastShowViews.showMyToastTwo("观看视频，点击下载广告", "12");
                        }
                    } else {
                        if (!CommonUtils.isDestory(MemberActivity.this)) {
    ...
    @OnClick({R.id.tv_level_reward, R.id.iv_backs, R.id.view, R.id.tv_unlocking, R.id.tv_gotoTixian, R.id.tv_baijinTask})
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_gotoTixian:
                ...
                break;
            case R.id.tv_baijinTask:
                if (baijinother_infos != null) {
                    unBaijinlockingDialog();
                } else {
                    ToastUtil.showToast("获取任务失败，请重新打开这个页面");
                }
                break;
        }
    }
    ...
    private BonusesInfoBeans bonuses_info;
    private int invite_meet_num;
    private List<TaskUnlock> baijinother_infos;
    ...
    private String baijinCash = "";

    @Override
    public void getbaijinUnlockTaskSuccess(TaskUnLockResBeans data) {
        baijinCash = "";
        List<TaskUnlock> data1ss = taskbaijingUnlockAdapte.getData();
        if (data1ss != null) {
            boolean isFinshAll = true;
            if (data1ss != null && data1ss.size() > 0) {
                for (int i = 0; i < data1ss.size(); i++) {
                    if (baijinunLockTaskId == data1ss.get(i).getOther_id()) {
                        data1ss.get(i).setFinish_num(data.getFinish_num());
                    }
                    if (data1ss.get(i).getNum() > data1ss.get(i).getFinish_num()) {
                        isFinshAll = false;
                    }
                }
            }
            if (taskbaijingUnlockAdapte != null) {
                taskbaijingUnlockAdapte.notifyDataSetChanged();
            }
            if (isFinshAll) {
                ToastUtil.showToast("白金会员任务完成");
                baijinCash = "1";
                if (tv_tixian != null) {
                    tv_tixian.setBackground(getResources().getDrawable(R.drawable.tv_red_ee492a));
                }
            } else {
                if (tv_tixian != null) {
                    tv_tixian.setBackground(getResources().getDrawable(R.drawable.tv_bg_gray11));
                }
                ToastUtil.showToast("请继续完成下一个任务哦！");
            }
        }
    ...
    @Override
    public void getbaijinUnlockTaskReeorState() {
        baijinCash = "";
    }

    @Override
    public void bonusescash(EmptyBeans data) {
        if (baijinunlockingDialog != null) {
            baijinunlockingDialog.setDismiss();
        }
        initWithDrawSuccess();
    }
    ...
    public void initWithDrawSuccess() {
        ...
        TextView tv_des1 = builder.findViewById(R.id.tv_des1);
        TextView tv_goWithDraw = builder.findViewById(R.id.tv_goWithDraw);
        tv_des1.setText("分红将会24小时内到账");
        tv_moneys.setText("请注意查收");
        tv_goWithDraw.setText("好的");
...
//        layoutParams.screenBrightness = WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
    ...
    private TaskUnlockAdapter taskbaijingUnlockAdapte;
    private BottomListDialog baijinunlockingDialog;
    private TextView tv_tixian;

    public void unBaijinlockingDialog() {
        baijinunlockingDialog = new BottomListDialog(this);
        View builder = baijinunlockingDialog.builder(R.layout.member_baijinunlocking_dialog);
        ImageView iv_close = builder.findViewById(R.id.iv_close);
        RecyclerView recyclerView = builder.findViewById(R.id.recyclerView);
        tv_tixian = builder.findViewById(R.id.tv_tixian);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(MemberActivity.this, LinearLayoutManager.VERTICAL, false);
        taskbaijingUnlockAdapte = new TaskUnlockAdapter(baijinother_infos);
        recyclerView.setLayoutManager(linearLayoutManager);
        recyclerView.setAdapter(taskbaijingUnlockAdapte);
        iv_close.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                baijinunlockingDialog.setDismiss();
        ...
        boolean isFinshAllss = true;
        if (baijinother_infos != null && baijinother_infos.size() > 0) {
            for (int i = 0; i < baijinother_infos.size(); i++) {
                if (baijinother_infos.get(i).getNum() > baijinother_infos.get(i).getFinish_num()) {
                    isFinshAllss = false;
                }
            }
        }
        tv_tixian.setText("分红奖励，每日可提");
        if (isFinshAllss) {//白金会员任务完成
            tv_tixian.setBackground(getResources().getDrawable(R.drawable.tv_red_ee492a));
        } else {
            tv_tixian.setBackground(getResources().getDrawable(R.drawable.tv_bg_gray11));
        }
        ...
        tv_tixian.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mPresenter.bonusescash(CacheDataUtils.getInstance().getUserInfo().getImei(), CacheDataUtils.getInstance().getUserInfo().getGroup_id());
            }
        });

        taskbaijingUnlockAdapte.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            @Override
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                List<TaskUnlock> data = adapter.getData();
                TaskUnlock taskUnlock = data.get(position);
                if (taskUnlock.getFinish_num() < taskUnlock.getNum()) {//未完成
                    baijinunLockTaskId = taskUnlock.getOther_id();
                    if (position == 0) {
                        MobclickAgent.onEvent(MemberActivity.this, "jiesuotask1");//参数二为当前统计的事件ID
                        videoType = 4;
                        if ("1".equals(AppSettingUtils.getVideoType())) {//先头条
        ...
        if (!CommonUtils.isDestory(MemberActivity.this)) {
            baijinunlockingDialog.setOutCancle(true);
            baijinunlockingDialog.setShow();
        }
    }
    ...
    public void shareWx(Bitmap resultPosterBitmap) {
        ...
        if (tempUri != null) {
            shareToWx(MemberActivity.this, tempUri);
        } else {
    ...
    public void shareToWx(Context context, Uri uri) {
        if (isInstalled(MemberActivity.this, "com.tencent.mm")) {
            try {
